...
export const Detail = () => {
  ...
  function handleClick(e) {
    ...
    e.preventDefault();
    fetch('http://127.0.0.1/increase_total_vote_count')
    for(var i = 0; i < moviegenres.length; i++){
    fetch('http://127.0.0.1/increase_vote_count/genre/' + moviegenres[i]['name'])
    }
    console.log(moviegenres[0]['name']);
    ...
    <>
      {
        loader ? <div className='h-screen w-full flex justify-center items-center'><span className="loader m-10"></span></div> :
          <>
            ...
            {/* watch movie */}
            <div className='flex justify-center items-center mb-10 gap-5 flex-wrap'>
              <Link  onClick={handleClick} className='flex border-2 border-green-600 bg-green-600/40 p-3 items-center justify-center gap-2 text-xl font-semibold rounded-full text-white'>
                <FaPlay />Vote
              </Link>
            </div>
          </>
